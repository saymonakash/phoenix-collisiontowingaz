...
function CostCalculator({ restrictToPhoenix = false }: { restrictToPhoenix?: boolean }) {
  // Service state
  ...
  const estimatedTotal = useMemo(() => {
    ...
    switch (service) {
      case "towing": {
        const perMileRate = vehicleInfo.isLarge
          ? PER_MILE_RATE_LARGE_VEHICLE_USD
          : PER_MILE_RATE_USD;
            ...
            <div className="grid lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-6">
                <LocationForm
                  ...
                  restrictToPhoenix={restrictToPhoenix}
                />
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              {!isReadyForQuote && (
                <p className="text-xs text-muted-foreground">
                  {!distanceCalculated ? "" : "Complete all required fields"}
                </p>
              )}
              <Button
                variant="secondary"
